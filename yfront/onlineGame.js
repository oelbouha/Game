...
class onlineGame extends game {
	constructor() {
		super();
		this.socket = new WebSocket('ws://127.0.0.1:8000/slap/match/');
	...
	async initGame() {
		...
		const playerOne = new CustomImage(STATIC_URL + "/assets/hands/hand1.png");
		const playerTwo = new CustomImage(STATIC_URL + "/assets/hands/hand1.png");
	...
	handleKeyPress(event) {
		...
		if (key == "ArrowDown")
		{
			...
			this.socket.send(JSON.stringify({ m: "retreat" }));
		}
		else if (key == "ArrowUp")
		{
			console.log("arrowup")
			this.socket.send(JSON.stringify({ m: "attack" }));
		}
...
// send hand index with opponent method